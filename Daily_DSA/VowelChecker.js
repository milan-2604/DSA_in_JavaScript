...
var spellchecker = function(wordlist, queries) {
    const result = [];
    //helper function
    function vowMask(word){
        return word.toLowerCase().replace(/[aeiou]/g,'*');
    }
    const exactSet = new Set();
    const caseInMap = new Map();
    const vowMaskedmap = new Map();

    for (let word of wordlist){
        exactSet.add(word);
        const lower = word.toLowerCase();
        const masked = vowMask(lower);
        if(!caseInMap.has(lower)){
            caseInMap.set(lower,word);
        }
        if(!vowMaskedmap.has(masked)){
            vowMaskedmap.set(masked,word);
        }
    }
    ...
    for(let query of queries){
        if(exactSet.has(query)){
            ...
            const masked = vowMask(lower);
            if(caseInMap.has(lower)){
                result.push(caseInMap.get(lower));
            }else if(vowMaskedmap.has(masked)){
                result.push(vowMaskedmap.get(masked));
            }else{
                result.push("");
            }
        }
